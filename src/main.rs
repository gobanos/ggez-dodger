...
impl event::EventHandler for MainState {
	...
	fn draw(&mut self, ctx: &mut Context) -> GameResult<()> {
		...
		if self.paused {
			graphics::set_color(ctx, Color::from_rgb(255, 255, 255))?;
			let text_pause = graphics::Text::new(ctx, "PAUSED",
												 &self.resources.font)?;
			let pos_x = ctx.conf.window_mode.width / 2 - self.resources.font.get_width("PAUSED")
				as u32
				/ 2;
			let pos_y = ctx.conf.window_mode.height / 2 - self.resources.font.get_height() as u32
				/ 2;

			graphics::draw(ctx, &text_pause, Point2::new(pos_x as f32, pos_y as f32), 0.)?;
		}
